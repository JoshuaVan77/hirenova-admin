...
export default function TopupRequests() {
  ...
  const [isRefreshing, setIsRefreshing] = useState(false); // Refresh State အသစ်

  // ၁။ Backend ကနေ Top-up Requests တွေကို ဆွဲယူခြင်း
  ...
  // ၂။ Refresh Function (Data အသစ်ပြန်ဆွဲယူခြင်း)
  const handleRefresh = async () => {
    setIsRefreshing(true);
    await fetchRequests();
    setTimeout(() => setIsRefreshing(false), 800); // Animation အတွက် ၀.၈ စက္ကန့် စောင့်မယ်
  };

  // ၃။ Approve / Reject လုပ်ခြင်း
  const handleAction = async (id, action) => {
    try {
      await axios.put(`${API_URL}/topup-requests/${id}`, {
        status: action,
        admin_note: '' 
      });
      
      // Local state ကို ချက်ချင်း Update လုပ်မယ်
      setRequests(requests.map(req => 
        req.id === id ? { ...req, status: action } : req
      ));
      
      alert(`Top-up request ${action} successfully!`);
    } catch (error) {
