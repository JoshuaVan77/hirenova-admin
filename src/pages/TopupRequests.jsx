...
export default function TopupRequests() {
  ...
  const fetchRequests = async () => {
    try {
      setError(null);
      const response = await axios.get(`${API_URL}/admin/topup-requests`, getAuthHeaders());
  ...
  const handleAction = async (id, action) => {
    ...
    if (!window.confirm(`Are you sure you want to ${action} this top-up request?`)) return;
    
    try {
      setError(null);
      await axios.put(
        `${API_URL}/admin/topup-requests/${id}`, 
        { status: action, admin_note: '' },
        getAuthHeaders()
      );
      
      setSuccessMsg(`Top-up request ${action} successfully!`);
